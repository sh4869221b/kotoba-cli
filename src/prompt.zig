...
pub fn build(allocator: std.mem.Allocator, source: lang.Language, target: lang.Language, mode: config.Mode, g: glossary.Glossary, text: []const u8) ![]const u8 {
    ...
    try buf.print("Translate from {s} to {s}. Return only the translation.\n", .{ source.asText(), target.asText() });
...
test "prompt includes glossary" {
    ...
    try std.testing.expectEqualStrings(
        "Translate from en to ja. Return only the translation.\n" ++
            "Preserve KOTOBA_PROTECT_* tokens exactly. Do not add commentary.\n" ++
            "Technical mode: preserve Markdown structure, identifiers, commands, code, paths, and technical meaning.\n" ++
            "Glossary:\n" ++
            "- CLI => CLI (protect)\n" ++
            "\nText:\n" ++
            "Hello `CLI`",
