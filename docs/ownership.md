...
| `cli.run` and public command adapters | `cli.run` owns an XDG/dispatch arena. Command adapters own invocation arenas where needed for argv-derived and command-local graphs; `doctor.run` owns the doctor diagnostic arena. | `xdg.Paths`, parsed arguments, loaded config/registry/glossary views, and adapter helper values are borrowed only during that call. | Every arena is deferred on success and error. `translate_cmd` passes the original allocator to `translate.run`, owns the returned `OwnedResult` until output/publication completes, and then deinitializes it. |
