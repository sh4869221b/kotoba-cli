...
| `cli.run` and public command adapters | `cli.run` owns an XDG resolution/dispatch arena. Command adapters own invocation arenas where needed for argv-derived and command-local graphs; `doctor.run` owns the doctor diagnostic arena. | `xdg.Resolution`, derived `xdg.Paths`, parsed arguments, loaded config/registry/glossary views, and adapter helper values are borrowed only during that call. | Every arena is deferred on success and error. Resolution only allocates derived paths and does not create filesystem entries; `ensureDirs` is the later writer-only mutation boundary. `translate_cmd` passes the original allocator to `translate.run`, owns the returned `OwnedResult` until output/publication completes, and then deinitializes it. |
