...
The current exact-SHA local receipts record 330 executed CPU tests with one
separate skip and 331 deterministic tests with zero skips; both profiles record
...
passing receipts. Current exact-SHA receipts record 369 cases: translate 50,
...
smoke run; `matrix` observes one common self-test and the 369-case matrix.
...
children, eight unit logs, two benchmarks, 30 measurements, 2,648 unit-test
executions, and four 369-case matrix receipts. Pull requests configure one
